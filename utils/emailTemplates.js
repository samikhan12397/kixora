...
export function orderConfirmationEmail(order) {
  ...
  return wrap(
    ...
    <p style="color:#8A8F98;">We'll email you again once your order ships.</p>
    `
  );
}
...
export function paymentPendingEmail(order) {
  return wrap(
    ...
    `
    <p>We've received your payment screenshot for order <strong>${order._id}</strong>.
    Our team is verifying it now — you'll get a confirmation email as soon as it's approved (usually within a few hours).</p>
    `
  );
}
...
export function paymentVerifiedEmail(order) {
  return wrap(
    ...
    We're preparing your order for shipping now!</p>
    `
  );
}